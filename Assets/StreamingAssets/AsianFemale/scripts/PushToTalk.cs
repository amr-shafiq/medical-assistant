...
public class PushToTalk : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transcriptionFilePath = basePath + "/transcription.txt";
        pythonSTTPath = basePath + "/scripts/azureSpeechSTT.py";
        pythonTTSPath = basePath + "/scripts/azureSpeechTTS.py";
        recordedFilePath = basePath + "/recorded1.wav";
        ttsAudioFilePath = basePath + "/tts_output.wav";
    ...
    void CopyFilesToPersistentPath()
    {
        ...
        string[] filesToCopy = {
            "recorded1.wav",
            "transcription.txt",
            "scripts/azureSpeechSTT.py",
            "scripts/azureSpeechTTS.py"
        };

        foreach (string fileName in filesToCopy)
        {
            string sourceFile = Path.Combine(Application.streamingAssetsPath, "AsianFemale", fileName);
            string destinationFile = Path.Combine(persistentPath, fileName);
            ...
            if (!File.Exists(destinationFile))
            {
                try
                {
                    ...
                    UnityEngine.Debug.Log($"Copied {fileName} to persistent path.");
                }
                catch (IOException e)
                {
                    UnityEngine.Debug.LogError($"Failed to copy {fileName}: {e.Message}");
                }
            }
        }
    }
    ...
    void StartSpeechRecognition()
    {
        ...
        {
        UnityEngine.Debug.LogError($"Error during speech recognition: {error}");
        }
        else
        {
        UnityEngine.Debug.Log($"Speech recognition output: {output}");
