...
def read_state_from_file():
    """
    Reads the state file and retrieves the current state, patient name, symptoms, and disease.
    """
    try:
        with open("H:/state.txt", "r") as file:
            lines = file.readlines()
            state_data = {}
            for line in lines:
                key, value = line.strip().split("=", 1)  # Handle cases where value may contain '='
...
# Example Usage
current_state, patient_name, symptoms, disease = read_state_from_file()
# print(f"State: {current_state}, Patient Name: {patient_name}")
...
def write_state_to_file(new_state, patient_name, symptoms=None, disease=""):
    ...
    try:
        ...
        # Write state to file
        with open("H:/state.txt", "w") as file:
...
async def assistant_conversation():
    ...
    transcription = await read_text_file("H:/transcription.txt")
    updated_symptoms = []
    ...
    if current_state == "waiting_for_name": 
        ...
        await text_to_speech_async(response, "H:/tts_output.wav")
